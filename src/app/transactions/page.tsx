    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-[1400px] mx-auto space-y-6 md:space-y-8">
      ...
      <main className="space-y-6 md:space-y-8">
        <div className="w-full max-w-4xl mx-auto space-y-4">
          <Suspense fallback={<div className="h-16 w-full bg-muted/20 animate-pulse rounded-xl border"></div>}>
            <ListFilter dict={dict} />
          </Suspense>
          <TransactionList user={user} dict={dict} limit={0} period={period} type={type} />
        </div>
      </main>
    </div>
  );
