...
export function DashboardSummary({
  ...
  const calculateStats = (txs: any[]) => {
    ...
    txs.forEach((tx) => {
      if (tx.type === "income") income += Number(tx.amount);
        ...
        broughtForward += Number(tx.amount);
    });
    return {
      income,
      expense,
      saving,
      broughtForward,
      balance: income + broughtForward - expense - saving,
    };
  };
  ...
  const monthStats = calculateStats(
    transactions.filter((tx) => tx.date.startsWith(currentMonthStr)),
  );
  const yearStats = calculateStats(
    transactions.filter((tx) => tx.date.startsWith(currentYearStr)),
  );
      ...
      <Card className="border-0 shadow-none md:border md:shadow-sm">
        ...
        <CardContent className="px-4 sm:px-6 py-4 sm:py-6 pt-0">
          ...
          <div className="text-xs text-muted-foreground mt-1 font-medium">
            ...
            ({dict.dashboard.cashflow || "Cash flow"})
          </div>
        </CardContent>
      </Card>
  ...
  const getActiveTransactions = () => {
    switch (activeTab) {
      ...
      case "year":
        return transactions.filter((tx) => tx.date.startsWith(currentYearStr));
      default:
        return transactions;
    }
  };
