...
export function DataActions({
  ...
  const handleExport = () => {
    ...
    link.setAttribute(
      "download",
      `cashcal-export-${new Date().toISOString().split("T")[0]}.csv`,
    );
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        if (results.data && results.data.length > 0) {
          ...
          if (
            confirm(
              dict.data.confirmImport.replace("{count}", count.toString()),
            )
          ) {
            setImporting(true);
            try {
              ...
              alert(
                dict.data.importSuccess.replace(
                  "{count}",
                  formattedData.length.toString(),
                ),
              );
            } catch (err) {
              console.error(err);
