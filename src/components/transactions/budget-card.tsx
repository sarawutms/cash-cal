...
export function BudgetCard({
  ...
  const [budget, setBudget] = useState<number>(
    user?.user_metadata?.monthly_budget || 0,
  );
  ...
  const percent =
    budget > 0 ? Math.min(100, (monthExpenses / budget) * 100) : 0;
    ...
    <Card className="border-0 shadow-none md:border md:shadow-sm w-full">
      <CardHeader className="px-0 md:px-6 pt-4 md:pt-6 flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-sm md:text-base font-medium">
          {dict.dashboard.budgetTitle || "Monthly Budget"}
        </CardTitle>
        <Dialog open={isEditing} onOpenChange={setIsEditing}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>
                {dict.dashboard.setBudget || "Set Monthly Budget"}
              </DialogTitle>
            </DialogHeader>
      ...
      <CardContent className="px-0 md:px-6 pb-6">
        {budget === 0 ? (
          <div className="text-sm text-muted-foreground">
            {dict.dashboard.noBudget ||
              "No budget set. Click the edit icon to set one."}
          </div>
        ) : (
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span
                className={`font-medium ${isOverBudget ? "text-rose-500" : isNearBudget ? "text-amber-500" : "text-emerald-500"}`}
              >
                {percent.toFixed(0)}%
              </span>
            </div>
