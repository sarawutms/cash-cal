...
export function TransactionForm({
  ...
  return (
    <FormWrapper
      className={isDialog ? "" : "border-0 shadow-none md:border md:shadow-sm"}
    >
      <HeaderWrapper className={isDialog ? "mb-4" : "px-0 md:px-6"}>
        <TitleWrapper className={isDialog ? "text-lg font-semibold" : ""}>
          {transaction
            ? dict.transaction.editTitle || "Edit Transaction"
            : dict.transaction.addTitle}
        </TitleWrapper>
      </HeaderWrapper>
        ...
        >
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder={dict.transaction.type}>
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${type === "income" ? "bg-emerald-500" : type === "expense" ? "bg-rose-500" : type === "brought_forward" ? "bg-slate-500" : "bg-indigo-500"}`}
                      />
                      {getTypeLabel(type)}
                    </div>
                  </SelectValue>
                ...
                <SelectContent>
                  ...
                  {(type === "brought_forward" ||
                    transaction?.type === "brought_forward") && (
          ...
          {user ? (
            <SubmitButton
              text={
                transaction
                  ? dict.transaction.update || "Update"
                  : dict.transaction.save
              }
              loadingText={dict.transaction.saving}
            />
          ) : (
            <LoginDialog
              trigger={
